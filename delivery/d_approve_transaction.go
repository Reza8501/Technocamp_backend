...
func (d *delivery) ApproveTransaction(c *gin.Context) {
	var request dto.ApproveTransaction
	...
	if userInfo["role"].(string) != "admin" {
		c.AbortWithStatusJSON(http.StatusUnauthorized, response.BuildUnauthorizedResponse("Invalid Authentication", "only admin can approve transaction"))
		return
	}

	result := d.use.ApproveTransaction(c.Request.Context(), request)
