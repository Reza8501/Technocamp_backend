...
func (u *usecase) CreateCourse(c context.Context, req dto.ReqCreateCourse) *response.ResponseContainer {
	...
	if errCreate != nil && gorm.IsRecordNotFoundError(errCreate) {
		...
		return response.BuildInternalErrorResponse(response.ERROR_CODE_DATABASE_ERROR, response.RESPONSE_CODE_INTERNAL_ERROR, "Error creating Cart", errCreate.Error())
	}
